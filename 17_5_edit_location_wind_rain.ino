...
// API|location|firmware
  ...
  shortKey[5] = '\0';
  strncpy(shortLocation, locationKey, 5);  
  shortLocation[5] = '\0';
  weatherText = "Api:" + String(shortKey)
              + "|location:" + String(shortLocation)
  ...
  tft.drawString(weatherText, 624, 60);
...
void loop() {
  ...
  if (currentMillis - lastCheckBlynk >= 2000) { // Check mỗi 2 giây
    ...
    if (!Blynk.connected()) {
      if (!BlynkErrorDrawn) { 
        ...
        tft.loadFont(AA_FONT_10, LittleFS);
        tft.setTextDatum(TL_DATUM);
        tft.setTextColor(TFT_BLACK, TFT_RED); // Chữ đen nền đỏ
...
//  Serial.println("🌙 Entering Light Sleep...");
  ...
  while (true) {
    if (digitalRead(SLEEP_PIN) == HIGH) {
      if (millis() - stableStart > 2000) break;
    } else {
      stableStart = millis();
    }
  delay(10);
...
void drawRain() {
  ...
  //  Rain Hourly
  tft.fillRect(560, 131, 3, 11, TFT_CYAN);  // for mmDaily data row
  tft.loadFont(AA_FONT_SMALL, LittleFS);  
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("888.88mm/d"));
  //  Rain Hourly
  tft.drawString(String(mmHourly, 2) + "mm/h", 566, 114);
  //  Rain Daily
  tft.drawString(String(mmDaily, 2) + "mm/d", 566, 130);
  //  Rain Total TFT
  tft.setTextPadding(tft.textWidth("88888.88 mm"));
  tft.drawString(String(mmTotal, 2) + "mm", 566, 146);
  tft.unloadFont();
  // RainGauge
  tft.fillRect(520, 63, 30, 100, 0x4228);                  //background GREY of bar graph
  tft.fillRect(520, 163 - mmGraph, 30, mmGraph, 0x07FF);  //bar graph CYAN
  for (int j = 0; j < 110; j = j + 10) {
    tft.drawLine(520, 163 - j, 549, 163 - j, TFT_BLACK);  // Line of RainGauge 0x07E00 == GREEN
  }
}
...
// Time Connect
  ...
  tft.drawString(String(fTimeConnect, 0) + "s", 800, 40);
  tft.unloadFont();
}
...
//  tft.fillRect(662, 219, 3, 12, TFT_DARKGREY);
  ...
  tft.drawString(String(Pressure + 6.8, 1) + "hPa", 542, 29);
  tft.drawString(String(Altitude, 1) + "m", 542, 41);
  tft.unloadFont();
}
  ...
  while (!validData && retry < 3) {
    ...
    if (httpResponseCode == HTTP_CODE_OK) {
      ...
      if (!error) {
        ...
        // Kiểm tra lỗi chuỗi phản hồi (Daily limit, endpoint...)
        // Nếu không phải JSON chuẩn, nó sẽ rơi vào đây
        // Ca có thể dùng http.getString() ở đây để log lỗi nếu muốn soi kỹ
      }
    }
...
void drawCurrentWeather(int h, int m) {
  ...
  String updatetime = "";
  updatetime += String(weatherData.timeupdate).substring(0, 5);
  tft.loadFont(AA_FONT_10, LittleFS);
  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(TFT_RED, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("12:30/12:00"));  // String width + margin
  tft.drawString("|" + timeup + "/" + updatetime, 800, 50);
//  String code = "";
//  code += String(currentWeather->weatherIcon).substring(0, 4);
//  tft.drawString("/" + code, 800, 40);
//  tft.drawString("20:20/20:00", 769, 60);
  String weathertest = "";
  weathertest = getMeteoconIcon(weatherData.weatherIcon, weatherData.Cloudcover, weatherData.isday);
...
//  String weatherText = "";
  weatherText = String(weatherData.temperature, 1);  // Make it integer temperature
  weatherText += "°";
...
void drawAstronomy(time_t local_time) {
  ...
  String sunrise12 = String(astronomyData.sunrise).substring(0, 8);
  String sunrise24 = convertTo24HourFormat(sunrise12);
  String sunset12  = String(astronomyData.sunset).substring(0, 8);
  ...
  String moonrise12 = String(astronomyData.moonrise).substring(0, 8);
  String moonrise24 = convertTo24HourFormat(moonrise12);
  String moonset12  = String(astronomyData.moonset).substring(0, 8);
