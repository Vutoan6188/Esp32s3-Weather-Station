...
//blynkservererror
bool blynkDisabled = false;
unsigned long lastCheckBlynk = 0;
...
void WeatherTask(void* pvParameters) {
  while (true) {
    ...
    // WeatherData
    if (firstWeatherRun || (m == 0 && s >= 3 && !GetWeatherData)) {
    ...
    // AstronomyData
    if (firstAstronomyRun || (h == 0 && m == 0 && s >= 9 && !GetAstronomy)) {
...
void loop() {
  ...
  // Check condition for Blynk.run()
  if (isAwake && wifiOK && !blynkDisabled && Blynk.connected()) {
    Blynk.run();
  }

  // Detect Blynk disconnect
  if (currentMillis - lastCheckBlynk >= 2000) {
    lastCheckBlynk = currentMillis;
    if (!blynkDisabled && !Blynk.connected()) {
      blynkDisabled = true;
      if (!blynkErrorDrawn) {
  ...
  // Reconnect blynk/minute
  if (blynkDisabled && isAwake && wifiOK) {
    if (m != lastReconnectMinute && currentMillis - lastReconnectTry > 1000) {
      lastReconnectMinute = m;
      lastReconnectTry = currentMillis;  // fix: dùng currentMillis thay vì 0
      if (Blynk.connect(500)) {          // fix: 500ms thay vì 1000ms
        blynkDisabled = false;
...
void espTemp(int baseY) {
  ...
  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(TFT_CYAN, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("100C"));
