...
void loop() {
  ...
  // Windy
  if (isAwake && wifiOK && s >= 20 && (m % 6 == 0) && lastWindyMinute != m) {
    lastWindyMinute = m;
    sendWindy();
  }
...
void sendWindy() {
  ...
  switch (httpCode) {
    ...
    case HTTP_CODE_OK:
      ...
      tft.drawString("WD", 690, yLocal);
      //Serial.println(F("[Windy] Upload OK"));
      break;

    case 400:
      tft.setTextColor(TFT_BLACK, TFT_RED);
      tft.drawString("WD", 690, yLocal);
      //Serial.println(F("[Windy] Invalid Payload"));
      break;

    case 401:
      tft.setTextColor(TFT_BLACK, TFT_RED);
      tft.drawString("WD", 690, yLocal);
      //Serial.println(F("[Windy] Password Error"));
      break;

    case 409:
      tft.setTextColor(TFT_BLACK, TFT_GREEN);
      tft.drawString("WD", 690, yLocal);
      //Serial.println(F("[Windy] Duplicate Upload"));
      break;

    case 429:
      tft.setTextColor(TFT_BLACK, TFT_RED);
      tft.drawString("WD", 690, yLocal);
      //Serial.println(F("[Windy] Rate Limit"));
      break;

    default:
      char codeBuf[8];
      snprintf(codeBuf, sizeof(codeBuf), "%d", httpCode);
      tft.setTextColor(TFT_BLACK, TFT_RED);
      tft.drawString(codeBuf, 690, yLocal);
      break;
  }
  tft.setTextPadding(0);
  tft.unloadFont();
