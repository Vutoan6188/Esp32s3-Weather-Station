...
void readSDData(time_t local_time) {
  ...
  tft.setCursor(0, 0);                     // Đặt con trỏ ở góc trên bên trái màn hình


  // 1. TỰ ĐỘNG PHỤC HỒI PHẦN CỨNG SD
  if (!sdReady) {
    tft.println("WARNING: The SD chua san sang! Dang thu reset...");
    SD.end();
    spiSD.end();
    delay(50);
    spiSD.begin(SD_SCK, SD_MISO, SD_MOSI, SD_CS);
  ...
  // 4. ĐỌC DỮ LIỆU BẰNG SSCANF VÀ IN RA TFT
  while (dataFile.available()) {
    ...
    float maxTempIn, minTempIn, tempIn, maxHumOut, minHumOut, humOut, maxHumIn, minHumIn, humIn;
    float windSpeed, windGust, windDir, rain, rainHourly, rainDailyVal;

    // Quét định dạng dấu gạch ngang '-'
    int parsedFields = sscanf(line.c_str(),
    ...
    // Nếu hụt, quét định dạng dấu gạch chéo '/'
    if (parsedFields < 5) {
      parsedFields = sscanf(line.c_str(),
                            "%d/%d/%d %d:%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f",
                            &fYear, &fMonth, &fDay, &fHour, &fMinute,
                            &maxTempOut, &minTempOut, &tempOutVal,
                            &maxTempIn, &minTempIn, &tempIn,
      ...
      if (fDay == targetDay) {
        if (fHour >= 0 && fHour < 24) {
          ...
          if (tft.getCursorY() > 760) {
            ...
            tft.println("...Va nhieu dong khac tiep theo...");
          }
        }
      }
    }
