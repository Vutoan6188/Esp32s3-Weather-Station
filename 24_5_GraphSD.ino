...
void readHistoryFromSD(time_t local_time);
void GraphWindRainSD();
void GraphTempSD();
void GraphHumSD();

void loadGraphFromSD(time_t local_time);
...
#define MAX_LAST_LINES 58
#define MAX_LINE_LENGTH 256

char lastLines[MAX_LAST_LINES][MAX_LINE_LENGTH];
...
int sdTempOut[MAX_SD_POINTS];
int sdTempIn[MAX_SD_POINTS];

int sdHumOut[MAX_SD_POINTS];
int sdHumIn[MAX_SD_POINTS];

int sdWindGust[MAX_SD_POINTS];
int sdWindDir[MAX_SD_POINTS];
int sdRain[MAX_SD_POINTS];

int sdValidPoints = 0;
//unsigned long lastLoraSimulate = 0;  // Lưu thời gian chạy giả lập gần nhất
...
void drawInterfaceSkeleton() {
  ...
  tft.drawLine(476, 57, 614, 57, 0x4228);
...
void loop() {
  ...
  // chỉ cập nhật time khi ở page 0
  if (currentPage == 0) {
...
void checkButton(time_t local_time) {
  ...
  if ((currentMillis - lastDebounceTime) > debounceDelay) {
    ...
    if (reading != buttonState) {
      ...
      // nhấn nút (INPUT_PULLUP => nhấn = LOW)
      if (buttonState == LOW) {
        ...
        if (currentPage == 0) {
          ...
          // vẽ lại giao diện trang 0
          drawInterfaceSkeleton();
          drawTime(local_time);
          drawData(TIMEZONE.toLocal(now(), &tz1_Code));
...
/*
void GetLoRa() {
  // --- A. LOGIC GIẢ LẬP DỮ LIỆU MỖI 4 GIÂY ---
  if (millis() - lastLoraSimulate >= 4000) {
    lastLoraSimulate = millis();  // Cập nhật lại mốc thời gian

    // 1. Hiển thị chữ LORA thông báo
    tft.loadFont(AA_FONT_10, LittleFS);
    tft.setTextDatum(TL_DATUM);
    tft.setTextColor(TFT_BLACK, TFT_CYAN);
    tft.setTextPadding(tft.textWidth("LORA"));
    tft.drawString("LORA", 624, 50);
    tft.unloadFont();  // Unload ngay sau khi vẽ chữ xong

    // 2. Giả lập thông số sóng RSSI
    dbm = -65;

    // 3. GIẢ LẬP DỮ LIỆU SỐ THỰC CHUẨN (Ép kiểu từ số nguyên)
    TempOutDoor = random(150, 380) / 10.0;  // Ngẫu nhiên từ 15.0 đến 38.0 độ C
    HumiOutDoor = random(40, 95);           // Ngẫu nhiên từ 40% đến 95%

    winddy = random(0, 360);             // Hướng gió 0 - 359 độ
    fWindSpeed = random(0, 150) / 10.0;  // Tốc độ gió 0.0 - 15.0 m/s
    RainGauge = random(0, 50) / 10.0;    // Lượng mưa từng đợt 0.0 - 5.0 mm

    fvalueBat = random(35, 42) / 10.0;  // Pin ngẫu nhiên 3.5V - 4.2V
    fvalueSol = random(0, 210) / 10.0;  // Solar ngẫu nhiên 0.0V - 21.0V

    fTimeConnect = random(100, 50000);
    Altitude = random(10, 500);
    Pressure = random(980, 1020);  // Áp suất khí quyển hPa
    ...
    newLoRaData = true;  // Kích hoạt drawData(local_time) trong loop()
  }

  // --- B. TỰ ĐỘNG XÓA CHỮ "LORA" SAU 1 GIÂY ĐỂ TRÁNH NHẤP NHÁY QUÁ NHANH ---
  // Nếu đã quá 1 giây kể từ lúc hiện chữ LORA, tiến hành xóa chữ đi
  if (millis() - lastLoraSimulate >= 1000 && millis() - lastLoraSimulate < 1100) {
    tft.fillRect(624, 50, 28, 11, TFT_BLACK);
  }
}
*/
...
void drawData(time_t local_time) {
  ...
  // --- ĐOẠN CUỐI HÀM drawData CỦA CA ---
  ResetValue(local_time);
  tft.setTextPadding(0);
  tft.unloadFont();  // Tắt font của phần vẽ dữ liệu cảm biến trước

  // Chu kỳ 6 phút ghi thẻ SD và vẽ đồ thị
...
void saveSDData(time_t local_time) {
  ...
  tft.drawString(String(logCount), 712, 50);
}
*/
void saveSDData(time_t local_time) {
  tft.loadFont(AA_FONT_10, LittleFS);
  ...
  char fileName[32];
  snprintf(fileName, sizeof(fileName), "/log_%04d_%02d.csv", year(local_time), month(local_time));
  ...
  if (!f) {
    ...
    spiSD.end();
    sdReady = false;
    digitalWrite(SD_CS, HIGH);  // Phòng thủ nhả bus
    return;
  }
  ...
  sprintf(datetime, "%04d-%02d-%02d %02d:%02d",
          ...
          hour(local_time), minute(local_time));
  ...
  tft.setTextColor(TFT_BLACK, TFT_GREEN);
  tft.drawString(" SD ", 690, 50);
  //Serial.println("✔️ Đã ghi dữ liệu vào thẻ SD thành công!");

  logCount++;

  // --- 6. IN SỐ LƯỢT GHI LOG CẠNH CHỮ SD ---
  char logBuf[8];
...
void readSDData(time_t local_time) {
  ...
  while (dataFile.available()) {
    ...
    if (parsedFields >= 24 && fMinute == 0) {

      // nếu đủ 58 dòng -> dồn lên
      if (storedLines >= MAX_LAST_LINES) {

        for (int i = 0; i < MAX_LAST_LINES - 1; i++) {

          strcpy(lastLines[i], lastLines[i + 1]);
        }

        storedLines = MAX_LAST_LINES - 1;
      }

      snprintf(lastLines[storedLines],
               MAX_LINE_LENGTH,
...
/*
bản này xài tạm ổn. nhiều điểm biểu đồ rơi xuống đáy
void readHistoryFromSD(time_t local_time) {
  ...
  float tempOutBuffer[MAX_SD_POINTS];
  float tempInBuffer[MAX_SD_POINTS];

  float humOutBuffer[MAX_SD_POINTS];
  float humInBuffer[MAX_SD_POINTS];

  float windGustBuffer[MAX_SD_POINTS];
  float windDirBuffer[MAX_SD_POINTS];

  float rainBuffer[MAX_SD_POINTS];
  ...
  while (dataFile.available()) {
    ...
    if (parsedFields >= 24 && fMinute == 0) {
      ...
      // =================================================
      // NẾU ĐỦ 800 ĐIỂM -> DỒN TRÁI
      // =================================================

      if (validCount >= MAX_SD_POINTS) {

        for (int i = 0; i < MAX_SD_POINTS - 1; i++) {

          tempOutBuffer[i] = tempOutBuffer[i + 1];
          tempInBuffer[i] = tempInBuffer[i + 1];

          humOutBuffer[i] = humOutBuffer[i + 1];
          humInBuffer[i] = humInBuffer[i + 1];

          windGustBuffer[i] = windGustBuffer[i + 1];
          windDirBuffer[i] = windDirBuffer[i + 1];

          rainBuffer[i] = rainBuffer[i + 1];
        }

        validCount = MAX_SD_POINTS - 1;
      }
      ...
      tempOutBuffer[validCount] = tempOutVal;
      tempInBuffer[validCount] = tempIn;

      humOutBuffer[validCount] = humOut;
      humInBuffer[validCount] = humIn;

      windGustBuffer[validCount] = windGust;
      windDirBuffer[validCount] = windDir;

      rainBuffer[validCount] = rainDailyVal;
  ...
  sdValidPoints = validCount;

  int startX = MAX_SD_POINTS - validCount;
  ...
  for (int i = 0; i < validCount; i++) {
    ...
    sdTempOut[x] = map(tempOutBuffer[i],
                       0,
                       50,
                       410,
                       360);

    sdTempIn[x] = map(tempInBuffer[i],
                      0,
                      50,
                      410,
                      360);
    ...
    sdHumOut[x] = map(humOutBuffer[i],
                      0,
                      100,
                      470,
                      420);

    sdHumIn[x] = map(humInBuffer[i],
                     0,
                     100,
                     470,
                     420);
    ...
    float gust = windGustBuffer[i];

    if (gust > 200) gust = 200;

    sdWindGust[x] = map(gust,
                        0,
                        200,
                        350,
                        250);
    ...
    sdWindDir[x] = map(windDirBuffer[i],
                       0,
                       360,
                       310,
                       270);
    ...
    float rainVal = rainBuffer[i];

    if (rainVal > 100) rainVal = 100;

    sdRain[x] = map(rainVal,
                    0,
                    100,
                    350,
                    250);
  ...
  Serial.print("Loaded SD points: ");
  Serial.println(sdValidPoints);
...
/*moi sua xong code nay khong bij roi bieu do nhuwng chua chay thu. gio laij lafm code moiws cho du 800pixels. docj ca du lieu thangs truowcs luoon
void readHistoryFromSD(time_t local_time) {
  ...
  while (dataFile.available()) {
    ...
    if (tempOutVal < -20 || tempOutVal > 60) {
      continue;
    }

    if (tempIn < -20 || tempIn > 60) {
      continue;
    }

    if (humOut < 0 || humOut > 100) {
      continue;
    }

    if (humIn < 0 || humIn > 100) {
      continue;
    }

    if (windGust < 0 || windGust > 250) {
      continue;
    }
    ...
    if (rainDailyVal < 0 || rainDailyVal > 500) {
      continue;
    }

    if (isnan(tempOutVal) || isnan(tempIn) || isnan(humOut) || isnan(humIn) || isnan(windGust) || isnan(windDir) || isnan(rainDailyVal)) {
      continue;
    }
  ...
  for (int i = 0; i < validCount; i++) {
    ...
    sdTempOut[x] = map(constrain(tempOutBuffer[i], 0, 50),
                       0,
                       50,
                       410,
                       360);

    sdTempIn[x] = map(constrain(tempInBuffer[i], 0, 50),
                      0,
                      50,
                      410,
                      360);
    ...
    sdHumOut[x] = map(constrain(humOutBuffer[i], 0, 100),
                      0,
                      100,
                      470,
                      420);

    sdHumIn[x] = map(constrain(humInBuffer[i], 0, 100),
                     0,
                     100,
                     470,
                     420);
    ...
    sdWindGust[x] = map(constrain(windGustBuffer[i], 0, 200),
                        0,
                        200,
                        350,
                        250);
    ...
    sdWindDir[x] = map(constrain(windDirBuffer[i], 0, 360),
                       0,
                       360,
                       310,
                       270);
    ...
    sdRain[x] = map(constrain(rainBuffer[i], 0, 100),
                    0,
                    100,
                    350,
                    250);
...
void readHistoryFromSD(time_t local_time) {
  ...
  for (int monthOffset = 1; monthOffset >= 0; monthOffset--) {
    ...
    int fileYear = year(local_time);
    int fileMonth = month(local_time) - monthOffset;
