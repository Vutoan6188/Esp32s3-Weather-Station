...
bool sdReady = false;
/*
// bien su dung string cho readSD

char lastLines[58][220];
int lineCount = 0;

*/
// bo string cho readSD
...
/*
code ok doc duoc nhiet do va van toc gio
void readSDData(time_t local_time) {
  ...
  if (!sdReady) {
    ...
    tft.println("WARNING: SD not ready!");
  ...
  if (!dataFile) {
    ...
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.println("ERROR: Open file failed!");
  ...
  while (dataFile.available()) {
    ...
    int parsedFields = sscanf(
                         line.c_str(),
                         "%d-%d-%d %d:%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f",
                         &fYear,
                         &fMonth,
                         &fDay,
                         &fHour,
    ...
    if (parsedFields >= 8) {
      ...
      if (fMinute == 0) {
        ...
        // lưu 58 dòng cuối
        strncpy(lastLines[lineCount % 58],
                buf,
                95);

        lastLines[lineCount % 58][95] = '\0';
...
*/
/*
code doc duoc het file roi. nhugn su dung string
void readSDData(time_t local_time) {
  ...
  while (dataFile.available()) {
    ...
    if (parsedFields >= 24) {
      ...
      if (fMinute == 0) {
        ...
        // =========================
        strncpy(
          lastLines[lineCount % 58],
          buf,
          sizeof(lastLines[0]) - 1);

        lastLines[lineCount % 58][sizeof(lastLines[0]) - 1] = '\0';
...
// =====================================================
// HÀM ĐỌC 58 DÒNG CUỐI FILE - KHÔNG DÙNG STRING
