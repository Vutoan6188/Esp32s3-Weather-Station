...
volatile bool AstronomyDataReady = false;
...
void WeatherTask(void* pvParameters) {
  ...
  while (true) {
    ...
    else if ((m == 0 || m == 30) && s >= 3 && (h != lastWeatherHour || m != lastWeatherMinute)) {
    ...
    else if (h == 0 && m == 0 && s >= 9 && d != lastAstronomyDay) {
...
void drawInterfaceSkeleton() {
  ...
  tft.drawLine(619, 0, 619, 108, 0x4228);
  ...
  tft.drawString("Heap", 624, 70);
  tft.setTextPadding(tft.textWidth("Min"));
  tft.drawString("Min", 624, 80);
  tft.setTextPadding(tft.textWidth("Block"));
  tft.drawString("Block", 624, 90);
  // Stack Free
  tft.drawString("Stack Free", 624, 100);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("Station Runtime"));
  tft.drawString("Station runtime", 624, 50);
...
void debugMemoryTFT(int baseY) {
  ...
  if (WeatherTaskHandle != NULL) {
    uint32_t stackRemain =
      uxTaskGetStackHighWaterMark(WeatherTaskHandle) * 4;
...
void loop() {
  ...
  // draw Astronomy
  if (AstronomyDataReady) {
    AstronomyDataReady = false;
...
void GetLoRa() {
  if (e220ttl.available() > 1) {
    ...
    tft.unloadFont();
    tft.fillRect(624, 40, 28, 11, TFT_BLACK);
  }
}
...
void updateBlynk() {
  ...
  tft.fillRect(654, 40, 36, 11, TFT_BLACK);
}
...
void saveSDData(time_t local_time) {
  ...
  tft.drawString(logBuf, 712, 40);

  debugMemoryTFT(currentPage == 0 ? 70 : 0);
}
...
*/
        // New code 800pixel error
        if (validCount >= MAX_SD_POINTS) {

          memmove(tempOutBuffer,
                  tempOutBuffer + 1,
                  (MAX_SD_POINTS - 1) * sizeof(int16_t));

          memmove(tempInBuffer,
                  tempInBuffer + 1,
                  (MAX_SD_POINTS - 1) * sizeof(int16_t));

          memmove(humOutBuffer,
                  humOutBuffer + 1,
                  (MAX_SD_POINTS - 1) * sizeof(int16_t));
          ...
          memmove(windGustBuffer,
                  windGustBuffer + 1,
                  (MAX_SD_POINTS - 1) * sizeof(int16_t));

          memmove(windDirBuffer,
                  windDirBuffer + 1,
                  (MAX_SD_POINTS - 1) * sizeof(int16_t));
