...
void debugMemoryTFT() {
  ...
  tft.drawString(buf, 748, 40);

  snprintf(buf, sizeof(buf), "%.1fk", minHeap / 1000.0);
  tft.drawString(buf, 748, 50);

  snprintf(buf, sizeof(buf), "%.1fk", largestBlock / 1000.0);
  tft.drawString(buf, 748, 60);

  // Free Heap
  tft.fillRect(750, 41, 50, 8, 0x4228);
  tft.fillRect(750, 42, barHeap, 6, TFT_GREEN);

  // Min Heap
  tft.fillRect(750, 51, 50, 8, 0x4228);
  tft.fillRect(750, 52, barMin, 6, TFT_GREEN);

  // Largest Block
  tft.fillRect(750, 61, 50, 8, 0x4228);
  tft.fillRect(750, 62, barBlock, 6, TFT_GREEN);
}
...
void loop() {
  ...
  //  WeatherData //
  if (booted || (m == 0 && s > 3 && !GetWeatherData)) {
  ...
  // AstronomyData //
  if (booted || (h == 0 && m == 1 && s > 6 && !GetAstronomy)) {
...
void GetLoRa() {
  if (e220ttl.available() > 1) {
    ...
    tft.fillRect(624, 70, 28, 11, TFT_BLACK);
  }
}
...
void ResetValue(time_t local_time) {
  ...
  if (h == 0 && m == 0 && s > 10 && d != lastResetDay) {
    lastResetDay = d;
...
void updateBlynk() {
  ...
  tft.fillRect(654, 70, 36, 11, TFT_BLACK);
}
...
void GraphWindRainSD() {
  ...
  tft.setTextPadding(tft.textWidth("8"));
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  tft.drawString("Wind & Rain", 0, 50);
  tft.drawString("Speed", 95, 50);
  tft.fillRect(80, 44, 10, 3, TFT_RED);
  tft.drawString("Direct", 195, 50);
  tft.fillRect(180, 44, 10, 3, TFT_OLIVE);
  tft.drawString("Rain", 295, 50);
  tft.fillRect(280, 44, 10, 3, TFT_CYAN);
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  //  tft.drawString("0", 0, 337);
...
void GraphTempSD() {
  ...
  tft.setTextDatum(BL_DATUM);
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  tft.drawString("Temperature", 0, 260);
  tft.drawString("Out", 95, 260);
  tft.fillRect(80, 254, 10, 3, TFT_ORANGE);
  tft.drawString("In", 195, 260);
  tft.fillRect(180, 254, 10, 3, TFT_CYAN);
  //  tft.setTextDatum(BL_DATUM);
  tft.setTextPadding(tft.textWidth("8"));
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  //  tft.drawString("0", 0, 407);
...
void GraphHumSD() {
  ...
  tft.setTextDatum(BL_DATUM);
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  tft.drawString("Humidity", 0, 370);
  tft.drawString("Out", 95, 370);
  tft.fillRect(80, 364, 10, 3, TFT_ORANGE);
  tft.drawString("In", 195, 370);
  tft.fillRect(180, 364, 10, 3, TFT_CYAN);
  tft.setTextDatum(BL_DATUM);
  tft.setTextPadding(tft.textWidth("8"));
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  //  tft.drawString("0", 0, 477);
