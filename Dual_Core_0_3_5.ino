...
volatile bool saveSDDone = false;
...
void debugMemoryTFT(int baseY) {
  ...
  int barBlock = map(
    ...
    */
  int barHeap = map(
    constrain(freeHeap, 40000, 100000),
    40000, 100000,
    0, 50);

  int barMin = map(
    constrain(minHeap, 5000, 25000),
    5000, 25000,
    0, 50);

  int barBlock = map(
    constrain(largestBlock, 15000, 45000),
    15000, 45000,
    0, 50);
  // =========================
  // PRINT TFT
...
void GetLoRa() {
  if (e220ttl.available() > 1) {
    ...
    loraDoc.clear();
    DeserializationError error = deserializeJson(loraDoc, rc.data);

    if (!error) {
      float xTempOutDoor = loraDoc["temperature"];
      if (!isnan(xTempOutDoor) && xTempOutDoor > 0 && xTempOutDoor < 80.0) {
        TempOutDoor = xTempOutDoor;
      }

      float xHumiOutDoor = loraDoc["humidity"];
      if (!isnan(xHumiOutDoor) && xHumiOutDoor > 0 && xHumiOutDoor <= 100.0) {
        HumiOutDoor = xHumiOutDoor;
      }

      winddy = loraDoc["angle"].as<float>();
      fWindSpeed = loraDoc["speed"].as<float>();
      RainGauge = loraDoc["rain"].as<float>();
      fvalueBat = loraDoc["battery"].as<float>();
      fvalueSol = loraDoc["solar"].as<float>();
      fTimeConnect = loraDoc["timerun"].as<float>();
      Altitude = loraDoc["altitude"].as<float>();
      Pressurebme = loraDoc["pressure"].as<float>();
...
void drawSignal() {
  ...
  // --- (Time Connect) ---
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("888.8d|888.8d"));
  float espRunDays = esp_timer_get_time() / 86400000000.0f;

  snprintf(sigBuf, sizeof(sigBuf),
           "%.1fd|%.1fd",
           fTimeConnect / 86400.0f,
           espRunDays);
  //  snprintf(sigBuf, sizeof(sigBuf), "%.1fd", fTimeConnect / 86400.0f);
...
void ResetValue(time_t local_time) {
  ...
  bool doHourReset = SendPWSDataDone && saveSDDone && h != lastResetHour;
  ...
  bool doDayReset = SendPWSDataDone && saveSDDone && d != lastResetDay;
...
void drawCurrentWeather(time_t local_time) {
  ...
  // Get hour from VisualCrossing
  int weatherHour = (weatherData.timeupdate[0] - '0') * 10 + (weatherData.timeupdate[1] - '0');
  // Same time Green, different time RED
  uint16_t updateColor = (weatherHour == h) ? TFT_GREEN : TFT_RED;

  tft.loadFont(AA_FONT_10, LittleFS);
  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(updateColor, TFT_BLACK);
...
void updateAstronomy(time_t local_time) {
  ...
  char url1d[280];
  snprintf(url1d, sizeof(url1d),
           "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/%s"
           "/today?unitGroup=metric&include=current&elements=moonrise,moonset,moonphase,sunrise,sunset&key=%s",
           locationKey, apiKey);
  /*
