...
String WUResponse = "";
volatile bool weatherCycleDone = false;  // core1 đọc, dùng để trigger resetValue()
...
void WeatherTask(void* pvParameters) {
  while (true) {
    ...
    // Chu kỳ 6 phút: WeatherData -> Windy -> WU
    if (m % 6 == 0 && s >= 2 && !SendPWSData) {

      // 1. WeatherData — 1 lần/giờ, track theo giờ thay vì khớp m==0 chính xác
      ...
      SendPWSData = true;
      weatherCycleDone = true;  // báo core1: chu kỳ vừa xong
    }
    if (m % 6 != 0)
      SendPWSData = false;

    // AstronomyData — 1 lần/ngày, track theo ngày
...
void ResetValue(time_t local_time) {
  ...
  if (m == 0 && s >= 11 && h != lastResetHour && weatherCycleDone) {
    lastResetHour = h;
    weatherCycleDone = false;
    mmHourly = 0;
    maxWindHour = 0;
  }
