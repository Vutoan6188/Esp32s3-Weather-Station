...
const char* GITHUB_REPO = "esp32-weather-ota";
const char* FIRMWARE_FILENAME = "otagithub.ino.bin";  // Tên file .bin trong phần release
...
void setup() {
  ...
  tft.drawString("Connecting to WiFi", 400, 260);
  ...
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.drawString("Location: " + String(locationKey), 200, 370);
  tft.drawString("Api Key: " + String(apiKey), 200, 390);
  tft.drawString("Blynk Auth Token: " + String(authKey), 200, 410);
...
//  delay(180000); //3 minute
  ...
  tft.fillRect(280, 206, 400, 320 - 206, TFT_BLACK);
...
//  Blynk.begin(BLYNK_AUTH_TOKEN, WIFI_SSID, WIFI_PASSWORD);
  ...
  tft.setTextPadding(240); // Pad next drawString() text to full width to over-write old text
  tft.drawString(" ", 400, 220);  // Clear line above using set padding width
...
/*  // Kiểm tra trạng thái nguồn mỗi 1 giây
  ...
  */// 🔄 Kiểm tra WiFi mỗi 10 phút
  ...
  //  loraE220();
  if (currentMillis - loraE220Millis >= 10) {
...
void checkFirmwareUpdate() {
  Serial.println("\n🔍 Kiểm tra firmware mới trên GitHub...");
  ...
  if (httpCode == 200) {
    ...
    Serial.printf("🔹 Phiên bản mới nhất: %s\n", latestVersion.c_str());
    Serial.printf("🔹 Phiên bản hiện tại: %s\n", FIRMWARE_VERSION);

    if (latestVersion != FIRMWARE_VERSION) {
      Serial.println("⚡ Có firmware mới — tiến hành OTA...");
      ...
      if (code == 200) {
        ...
        if (Update.begin(len)) {
          ...
          if (written == len) {
            Serial.println("✅ Ghi xong firmware!");
          } else {
            Serial.printf("⚠️ Ghi thiếu dữ liệu! (%d/%d bytes)\n", written, len);
          }

          if (Update.end()) {
            if (Update.isFinished()) {
              Serial.println("🎉 Update thành công — rebooting...");
              ESP.restart();
            } else {
...
void loraE220(int h, int m, int s) {   
  if (e220ttl.available()>1) {
    ...
    dbm = - (256 - rssi);
    WiFiRSSI = WiFi.RSSI();
    String jsonString = rc.data;
    StaticJsonDocument<250> doc;
    DeserializationError error = deserializeJson(doc, jsonString); 
//    Serial.println(jsonString);      
  //  Get Temp/Hum OUT
    float xTempOutDoor = doc["temperature"];
      if (!isnan(xTempOutDoor) && xTempOutDoor > 0 && TempInDoor < 80.0) {
...
//  4s
    if (currentMillis - prev3s > 3000)  {
      prev3s = currentMillis;
...
void drawWindSpeed() {
  ...
  if (fWindSpeed > maxWindHour)
    maxWindHour = fWindSpeed;
...
void drawAngle() {
  ...
  tft.loadFont(AA_FONT_SMALL, LittleFS);
  tft.setTextDatum(TL_DATUM);
  tft.setTextPadding(tft.textWidth("WIND"));
...
//  tft.fillRect(662, 219, 3, 12, TFT_DARKGREY);
  ...
  //  Press && Altitude
  ui.drawBmp("/icon50/Altitude.bmp", 355, 4);  
  ui.drawBmp("/icon50/cushin.bmp", 704, 72);  
  tft.setTextPadding(tft.textWidth("8888.8hPa"));
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(String(Pressure + 6.8, 1) + "hPa", 427, 29);
  tft.drawString(String(Altitude, 1) + "m", 427, 41);  
}

void drawRain() {
  // Rain Counts
  if (RainGauge != prevTotal) {
    mmTotal = mmTotal + RainGauge;
    mmDaily = mmDaily + RainGauge;
    mmHourly = mmHourly + RainGauge;
  }
  prevTotal = RainGauge;
  if (mmDaily > 99) mmDaily = 0;
  //        mmDaily = mmDaily + 2;
  //  Rain Hourly
  tft.fillRect(374, 131, 3, 11, TFT_CYAN);
  tft.loadFont(AA_FONT_SMALL, LittleFS);
//  tft.setTextDatum(TL_DATUM);
//  tft.setTextPadding(tft.textWidth("RAIN"));
//  tft.setTextColor(TFT_GREENYELLOW, TFT_BLACK);
//  tft.drawString("RAIN", 370, 64);
  ...
  // Rain Total TFT
  //tft.setTextColor(TFT_CYAN, TFT_BLACK); //#5F9EA0  CadetBlue (W3C)
  ...
  tft.fillRect(337, 163 - mmDaily, 30, mmDaily, 0x07FF);  //bar graph CYAN
  tft.drawLine(335, 63, 335, 163, 0x4228);
  tft.drawLine(368, 63, 368, 163, 0x4228);
...
void GraphWindRain() {
  ...
  tft.fillRect(0, 302, 800, 7, 0x3186);
  tft.fillRect(0, 282, 800, 7, 0x3186);
//  Graph Line
...
//  N E S W N
  ...
  graphRain = map(mmDaily, 0, 100, 350, 250);
    b[800] = graphRain;
...
void GraphTemp() {
  ...
  tft.fillRect(0, 372, 800, 7, 0x3186);
  tft.fillRect(0, 392, 800, 7, 0x3186);    
//  Graph Line
...
void GraphHum() {
  ...
  tft.fillRect(0, 432, 800, 7, 0x3186);
  tft.fillRect(0, 452, 800, 7, 0x3186);    
//  Graph Line
  for (int m = 0; m < 60; m = m + 10) {
...
void drawCurrentWeather(int h, int m) {
  ...
  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(TFT_RED, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("88:88/88:88"));  // String width + margin
  tft.drawString(timeup + "/" + updatetime, 769, 40);
  String code = "";
...
//  tft.drawString("20:20/20:00", 769, 60);
  ...
  weatherText = currentWeather->weatherStat + "|Api:" + shortKey;
...
***************************************************************************************/
void drawAstronomy() {
/*  String updatetimemoon = "";
  if (h < 10) updatetimemoon += "0";
  updatetimemoon += h;
  updatetimemoon += ":";
  if (m < 10) updatetimemoon += "0";
  updatetimemoon += m;
  tft.loadFont(AA_FONT_10, LittleFS);
  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(TFT_RED, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("44:44"));  // String width + margin
  tft.drawString("/" + updatetimemoon, 800, 40);
*/
  ...
  if (dailyWeather->moonage > 0.02) moonage++;
  if (moonage > 29) moonage = 29;
  ui.drawBmp("/icon50/calendar.bmp", 255, 178);
  tft.loadFont(AA_FONT_20, LittleFS);
  tft.drawString(String(moonage), 279, 205);
  ui.drawBmp("/moon/" + String(moonage) + ".bmp", 90, 167);
...
//  tft.drawString("12:34", 40, 207);
//  tft.drawString("21:00", 40, 248);     
...
//  tft.drawString("12:34", 219, 207);
//  tft.drawString("21:00", 219, 248);    
}
...
void drawProgress(uint8_t percentage, String text) {
  ...
  ui.drawProgressBar(290, 269, 240 - 20, 15, percentage, TFT_DARKGREY, TFT_CYAN);
