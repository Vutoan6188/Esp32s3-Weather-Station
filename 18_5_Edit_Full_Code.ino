...
void setup() {
  ...
  maxTempInDoor = 0;   minTempInDoor = 99;   maxHumiInDoor = 0;   minHumiInDoor = 99;
  maxTempOutDoor = 0;  minTempOutDoor = 99;  maxHumiOutDoor = 0;  minHumiOutDoor = 99;
  mmHourly = 0;        mmDaily = 0;          mmGraph = 0;         maxWindHour = 0;
  valueLoRaStrong = 0; valueWiFiStrong = 0;  valueBattery = 0;    valueSolar = 0;

  // --- SỬA LỖI TRÀN MẢNG ĐỒ THỊ ---
  // Mảng có 800 phần tử thì index chạy từ 799 về 0. Để i = 800 sẽ bị sập RAM đột ngột!
  for (int i = 799; i >= 0; i--) {
    x[i] = 9999; y[i] = 9999; z[i] = 9999; a[i] = 9999; b[i] = 9999; g[i] = 9999; // Graph
    c[i] = 9999; d[i] = 9999; v[i] = 9999; f[i] = 9999;                         // Graph1
    n[i] = 9999; h[i] = 9999; m[i] = 9999; j[i] = 9999;                         // Graph2      
  }
...
void saveData(time_t local_time) {
  ...
  f.close();
  logCount++;
// --- 4. IN SỐ LƯỢT GHI LOG (ĐÃ SỬA: SẠCH CHỮ STRING) ---
  char logBuf[8];
  snprintf(logBuf, sizeof(logBuf), "%d", logCount); // Ép số logCount thành chuỗi tĩnh
  
  tft.setTextDatum(TL_DATUM);    
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.setTextPadding(tft.textWidth("9999"));  
  tft.drawString(logBuf, 712, 50); // In số lượt ghi kế bên chữ SD
